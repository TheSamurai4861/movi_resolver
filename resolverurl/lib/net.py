...
def get_ua():
    ua = UserAgent()
    user_agent = ua.chrome  # Vous pouvez spécifier 'chrome', 'firefox', 'safari', etc.
    return user_agent
...
class NoRedirection(HTTPRedirectHandler):
    def http_error_302(self, req, fp, code, msg, headers):
        infourl = addinfourl(fp, headers, req.get_full_url() if six.PY2 else req.full_url)
...
class Net:
    ...
    def _update_opener(self):
        ...
        """
        handlers = [HTTPCookieProcessor(self._cj), HTTPBasicAuthHandler()]

        if self._http_debug:
            handlers += [HTTPHandler(debuglevel=1)]
        else:
            handlers += [HTTPHandler()]

        if self._proxy:
            handlers += [ProxyHandler({'http': self._proxy})]
    ...
    def _fetch(self, url, form_data={}, headers={}, compression=True, jdata=False, redirect=True):
        ...
        if form_data:
            ...
            req = Request(url, data=form_data)
        ...
        try:
            # Créer un contexte SSL avec un niveau de sécurité inférieur
            context = ssl.create_default_context()
            context.set_ciphers('DEFAULT@SECLEVEL=1')

            if not redirect:
                opener = build_opener(NoRedirection(), HTTPSHandler(context=context))
                response = opener.open(req, timeout=20)
            else:
                response = urlopen(req, context=context, timeout=15)
        except HTTPError as e:
            if e.code == 403 and 'cloudflare' in e.hdrs.get('server', ''):
                # Gérer les erreurs 403 de Cloudflare avec un contexte SSL ajusté
